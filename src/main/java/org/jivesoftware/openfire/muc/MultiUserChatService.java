...
public interface MultiUserChatService extends Component {
     ...
     * 
     * @return the chat server domain (service name + domain name).
     */
    String getServiceDomain();

    /**
     * Returns the top level domain of this service.  This is effective the service domain minus the service name.
     * @return
     */
    String getDomain();
    ...
    default boolean isPasswordRequiredForSysadminsToJoinRoom() {
        return MUCPersistenceManager.getBooleanProperty( getServiceName(), getDomain(), "sysadmin.requires.room.passwords", false );
    }
    ...
    default void setPasswordRequiredForSysadminsToJoinRoom(boolean isRequired) {
        MUCPersistenceManager.setProperty( getServiceName(), getDomain(), "sysadmin.requires.room.passwords", Boolean.toString(isRequired) );
    }
