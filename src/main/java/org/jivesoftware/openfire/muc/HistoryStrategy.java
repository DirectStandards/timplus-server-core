...
public class HistoryStrategy {
    ...
    public void setMaxNumber(int max) {
        ...
        if (contextPrefix != null){
            MUCPersistenceManager.setProperty(contextSubdomain, domain, contextPrefix + ".maxNumber", Integer.toString(maxNumber));
        }
        if (parent == null) {
            // Update the history strategy of the MUC service
            CacheFactory.doClusterTask(new UpdateHistoryStrategy(contextSubdomain, domain, this));
        }
    }
    ...
    public void setType(Type newType){
        ...
        if (contextPrefix != null){
            MUCPersistenceManager.setProperty(contextSubdomain, domain, contextPrefix + ".type", type.toString());
        }
        if (parent == null) {
            // Update the history strategy of the MUC service
            CacheFactory.doClusterTask(new UpdateHistoryStrategy(contextSubdomain, domain, this));
     ...
     */
    public void setContext(String subdomain, String domain, String prefix) {
        this.contextSubdomain = subdomain;
        this.domain = domain;
        this.contextPrefix = prefix;
        setTypeFromString(MUCPersistenceManager.getProperty(subdomain, domain, prefix + ".type"));
        String maxNumberString = MUCPersistenceManager.getProperty(subdomain, domain, prefix + ".maxNumber");
