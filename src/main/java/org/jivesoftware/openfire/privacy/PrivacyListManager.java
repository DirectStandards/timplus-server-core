...
public class PrivacyListManager {
     ...
     */
    public PrivacyList createPrivacyList(String username, String domain, String listName, Element listElement) {
        // Create new list
        PrivacyList list = new PrivacyList(username, domain, listName, false, listElement);
     ...
     *         none was found.
     */
    public PrivacyList getDefaultPrivacyList(String username, String domain) {
        ...
        if (list == null) {
            synchronized ((username + MUTEX_SUFFIX).intern()) {
                ...
                if (list == null) {
                    ...
                    list = provider.loadDefaultPrivacyList(username, domain);
                    if (list != null) {
     ...
     *         none was found.
     */
    public PrivacyList getPrivacyList(String username, String domain, String listName) {
        // Check if we have a list in the cache
        String cacheKey = getCacheKey(username, listName);
        PrivacyList list = listsCache.get(cacheKey);
        if (list == null) {
            // Load the list from the database
            list = provider.loadPrivacyList(username, domain, listName);
            if (list != null) {
                listsCache.put(cacheKey, list);
            }
        }
