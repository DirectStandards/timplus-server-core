...
public class RosterManager extends BasicModule implements GroupEventListener, UserEventListener {
     ...
     */
    public Roster getRoster(String username, String domain) throws UserNotFoundException {
        ...
        if (roster == null) {
           ...
           synchronized ((username  + MUTEX_SUFFIX).intern()) {
                ...
                if (roster == null) {
                    ...
                    roster = new Roster(username, domain);
        ...
        try {
            ...
            while (usernames.hasNext()) {
                ...
                try {
                    ...
                    roster = getRoster(username, user.getDomain());
     ...
     */
    public Collection<Group> getSharedGroups(String username, String domain) {
        Collection<Group> answer = new HashSet<>();
        Collection<Group> groups = GroupManager.getInstance().getSharedGroups(username, domain);
        for (Group group : groups) {
            String showInRoster = group.getProperties().get("sharedRoster.showInRoster");
            if ("onlyGroup".equals(showInRoster)) {
                if (group.isUser(username, domain)) {
                    ...
                    for (Group groupInList : groupList) {
                        if (groupInList.isUser(username, domain)) {
                            answer.add(group);
                        }
                    }
                }
    ...
    public void userCreated(User newUser, Map<String,Object> params) {
        JID newUserJID = server.createJID(newUser.getUsername(), newUser.getDomain(), null);
    ...
    public void userDeleting(User user, Map<String,Object> params) {
        ...
        JID userJID = server.createJID(user.getUsername(), user.getDomain(), null);
        // Shared public groups that are public should have a presence subscription
        // of type FROM for the new user
        for (Group group : getPublicSharedGroups()) {
            // Get group members of public group
    ...
    public void userModified(User user, Map<String,Object> params) {
        if ("nameModified".equals(params.get("type"))) {
            ...
            for (Group group : getSharedGroups(user.getUsername(), user.getDomain())) {
    ...
    Collection<JID> getSharedUsersForRoster(Group group, Roster roster) {
        ...
        if (group.isUser(roster.getUsername(), DomainResolver.resolveUserDomain(roster.getUsername())  )) {
            // Check if anyone can see this shared group
            if ("everybody".equals(showInRoster)) {
                // Add all users in the system
                for (String username : UserManager.getInstance().getUsernames()) {
    ...
    boolean hasMutualVisibility(String user, Collection<Group> groups, JID otherUser,
        ...
        for (Group group : groups) {
            for (Group otherGroup : otherGroups) {
                ...
                if (!group.isUser(user, DomainResolver.resolveUserDomain(user)) || !otherGroup.isUser(otherUser)) {
                    continue;
                }
                        ...
                        for (Group groupName : groupList) {
                            if (groupName.isUser(otherUser)) {
                                for (Group otherGroupName : otherGroupList) {
                                    if (otherGroupName.isUser(user, DomainResolver.resolveUserDomain(user))) {
                                        return true;
                                    }
                                }
                            }
