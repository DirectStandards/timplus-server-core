...
public class Roster implements Cacheable, Externalizable {
     ...
     */
    Roster(String username, String domain) 
    {
        ...
        Collection<Group> sharedGroups = rosterManager.getSharedGroups(username, domain);
    ...
    public RosterItem deleteRosterItem(JID user, boolean doChecking) throws SharedGroupException {
        ...
        if (itemToRemove != null) {
            ...
            if (subType == RosterItem.SUB_TO || subType == RosterItem.SUB_BOTH) {
                ...
                presence.setFrom(XMPPServer.getInstance().createJID(username, domain, null));
    ...
    public void broadcastPresence(Presence packet) {
        ...
        if (list == null) {
            ...
            list = PrivacyListManager.getInstance().getDefaultPrivacyList(username, domain);
        }
    ...
    private void broadcast(org.xmpp.packet.Roster roster) {
        JID recipient = XMPPServer.getInstance().createJID(username, domain, null, true);
    ...
    void addSharedUser(Group group, JID addedUser) {
        ...
        // user and this user
        else if (group.isUser(addedUser) && !group.isUser(getUsername(), domain)) {
    ...
    private JID getUserJID() {
        return XMPPServer.getInstance().createJID(getUsername(), domain, null, true);
    }
