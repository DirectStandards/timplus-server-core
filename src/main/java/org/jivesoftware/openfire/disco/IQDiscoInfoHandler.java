...
public class IQDiscoInfoHandler extends IQHandler implements ClusterEventListener {
    ...
    public IQ handleIQ(IQ packet) {
        ...
        if (infoProvider != null) {
            ...
            String domain = packet.getTo() == null ? null : packet.getTo().getDomain();
            ...
            if (infoProvider.hasInfo(name, node, packet.getFrom(), domain)) {
                reply.setChildElement(iq.createCopy());
                Element queryElement = reply.getChildElement();

                // Add to the reply all the identities provided by the DiscoInfoProvider
                Element identity;
                Iterator<Element> identities = infoProvider.getIdentities(name, node, packet.getFrom(), domain);
                ...
                Iterator<String> features = infoProvider.getFeatures(name, node, packet.getFrom(), domain);
                ...
                final Set<DataForm> dataForms = infoProvider.getExtendedInfos( name, node, packet.getFrom() , domain);
                if ( dataForms != null ) {
    ...
    protected void setProvider(String name, DiscoInfoProvider provider) {
    	Log.info("Adding DiscoInfoOProvider with name " + name);
    ...
    protected void removeProvider(String name) {
    	Log.info("Removing DiscoInfoOProvider with name " + name);
    ...
    public void initialize(XMPPServer server) {
        ...
        for (String domainName : DomainManager.getInstance().getDomainNames(true))
        	setProvider(domainName, getServerInfoProvider());
        ...
        return new DiscoInfoProvider() {
            ...
            public Iterator<Element> getIdentities(String name, String node, JID senderJID, String domain) {
                if (node != null && serverNodeProviders.get(node) != null) {
                    ...
                    return serverNodeProviders.get(node).getIdentities(name, node, senderJID, domain);
                }
                if (name == null || DomainManager.getInstance().isRegisteredDomain(domain)) {
                    ...
                    return XMPPServer.getInstance().getIQPEPHandler().getIdentities(name, node, senderJID, domain);
                }
                else {
            ...
            public Iterator<String> getFeatures(String name, String node, JID senderJID, String domain) {
                if (node != null && serverNodeProviders.get(node) != null) {
                    // Redirect the request to the disco info provider of the specified node
                    return serverNodeProviders.get(node).getFeatures(name, node, senderJID, domain);
                }
                if (name == null || DomainManager.getInstance().isRegisteredDomain(name)) {
                    // Answer features of the server itself.
                    return new HashSet<>(serverFeatures.keySet()).iterator();
                }
                else if (node != null) {
                    return XMPPServer.getInstance().getIQPEPHandler().getFeatures(name, node, senderJID, domain);
                }
                else {
            ...
            public boolean hasInfo(String name, String node, JID senderJID, String domain) {
                if (node != null) {
                    if (serverNodeProviders.get(node) != null) {
                        // Redirect the request to the disco info provider of the specified node
                        return serverNodeProviders.get(node).hasInfo(name, node, senderJID, domain);
                    }
                    if (name != null) {
                        return XMPPServer.getInstance().getIQPEPHandler().hasInfo(name, node, senderJID, domain);
                    }
                    // Unknown node
                    return false;
                }
            ...
            public Set<DataForm> getExtendedInfos(String name, String node, JID senderJID, String domain) {
                if (node != null && serverNodeProviders.get(node) != null) {
                    // Redirect the request to the disco info provider of the specified node
                    return serverNodeProviders.get(node).getExtendedInfos(name, node, senderJID, name);
                }
                if (name == null || DomainManager.getInstance().isRegisteredDomain(name)) {
                ...
                if (node != null && name != null) {
                    return XMPPServer.getInstance().getIQPEPHandler().getExtendedInfos(name, node, senderJID, domain);
                }
