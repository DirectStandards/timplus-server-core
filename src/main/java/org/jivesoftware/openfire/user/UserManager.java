...
public final class UserManager {
            ...
            throws UserAlreadyExistsException, DomainNotFoundException
    {
        ...
        catch (final UnsupportedEncodingException se) {
            throw new IllegalArgumentException("Invalid encoded username: " + username,  se);
        }
        ...
        final User user = provider.createUser(username, password, domain, name, email);
        userCache.put(user.getUsername(), user);
    ...
    public User getUser(String username) throws UserNotFoundException {
        ...
        if (user == null) {
            synchronized ((username + MUTEX_SUFFIX).intern()) {
                user = userCache.get(new StringBuilder(username + MUTEX_SUFFIX).toString().intern());
                if (user == null) {
                    user = provider.loadUser(username);
                    userCache.put(new StringBuilder(user.getUsername()).toString(), user);
                }
            }
        }
        return user;
    ...
    /**
     * Returns an unmodifiable Collection of all users in a domain
     *
     * @param domainName The domain Name to search for
     * @return an unmodifiable Collection of all users in a domain
     */
    public Collection<User> getUsersByDomain(String domainName)
    {
        ...
        return provider.getUsersByDomain(domainName);
    }
