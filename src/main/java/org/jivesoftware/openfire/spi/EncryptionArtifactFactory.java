...
public class EncryptionArtifactFactory
{
    ...
    private static SslFilter createSslFilter( SSLContext sslContext, SSLEngine sslEngine ) {
        final SslFilter filter = new SslFilter( sslContext )
        {
        	...
        	{
        		// In accordance to RFC 6120, server connections SHOULD use the "from" domain (in this case the connection domain)
        		// as the reference id for selecting the server certificate
        		final Connection conn = (Connection)session.getAttribute("CONNECTION");
        		
        		if (conn != null)
        			ReferenceIDUtil.setSessionReferenceId(sslEngine.getSession(), conn.getConnectionDomain());
