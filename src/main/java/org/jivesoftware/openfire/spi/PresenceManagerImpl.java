...
public class PresenceManagerImpl extends BasicModule implements PresenceManager, UserEventListener, XMPPServerListener {
    ...
    @Override
    public boolean canProbePresence(JID prober, String probee, String domain) throws UserNotFoundException {
        if (probee.equals(prober.getNode()) && XMPPServer.getInstance().isLocal(prober)) {
            return true;
        }
        RosterItem item = rosterManager.getRoster(probee, domain).getRosterItem(prober);
