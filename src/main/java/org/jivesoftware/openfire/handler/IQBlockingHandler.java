...
public class IQBlockingHandler extends IQHandler implements ServerFeaturesProvider
{
    ...
    protected Set<JID> getBlocklist( User user )
    {
        ...
        final PrivacyList defaultPrivacyList = PrivacyListManager.getInstance().getDefaultPrivacyList( user.getUsername() , user.getDomain());
    ...
    protected void addToBlockList( User user, List<JID> toBlocks )
    {
        ...
        if ( defaultPrivacyList == null )
        {
            ...
            defaultPrivacyList = PrivacyListManager.getInstance().createPrivacyList( user.getUsername(), user.getDomain(), "blocklist", listElement );
