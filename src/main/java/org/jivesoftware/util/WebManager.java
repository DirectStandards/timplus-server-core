...
public class WebManager extends WebBean {
    ...
    {
    	final Collection<Domain> domains = getServerInfo().getXMPPDomains(true);
    	...
    	while (iter.hasNext())
    	{
    		builder.append(iter.next().getDomainName());
    		if (iter.hasNext())
    			builder.append(", ");
    	}
    ...
    public User getUser() {
        ...
        try {
            ...
            if (authToken instanceof AuthToken.OneTimeAuthToken) 
            {
            	...
            	String domain = DomainResolver.resolveUserDomain(username);
            	
                return new User(authToken.getUsername(), "Recovery via One Time Auth Token", null, domain, new Date(), new Date());
            }
            final String username = authToken.getUsername();
        	String domain = DomainResolver.resolveUserDomain(username);
