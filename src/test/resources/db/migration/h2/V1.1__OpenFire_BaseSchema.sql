...
CREATE TABLE ofUser (
  ...
  CONSTRAINT ofUser_pk PRIMARY KEY (username, domain)
);
...
CREATE TABLE ofDomain (
  domainName            VARCHAR(64)     NOT NULL,
  enabled               bit(1),
  creationDate          VARCHAR(15)     NOT NULL,
  modificationDate      VARCHAR(15)     NOT NULL,
  CONSTRAINT ofDomain_pk PRIMARY KEY (domainName)
);
CREATE INDEX ofDomain_cDate_idx ON ofUser (creationDate);
...
CREATE TABLE ofMucService (
  ...
  CONSTRAINT ofMucService_pk PRIMARY KEY (subdomain, domain)
);
...
INSERT INTO ofUser (username, plainPassword, name, email, creationDate, modificationDate, domain)
    VALUES ('admin', 'admin', 'Administrator', 'admin@example.com', '0', '0', 'example.com');
